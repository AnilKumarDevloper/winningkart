...
//Shows Price on page based on low to high
if (!function_exists('home_price_new')){
    function home_price_new($product, $formatted = true){
        $unit_price = $product->unit_price;
        if($product->variant_product){
            $unit_price = $product->stocks[0]->price; 
        }
        // foreach($product->taxes as $product_tax){
        //     if($product_tax->tax_type == 'percent'){
        //         $lowest_price += ($lowest_price * $product_tax->tax) / 100;
        ...
        // }
        if($formatted){
            return format_price(convert_price($unit_price));
        }
    }
}

//Shows Price on page based on low to high with discount
if(!function_exists('home_discounted_price_new')){
    function home_discounted_price_new($product, $formatted = true){
        $unit_price = $product->unit_price;
        if ($product->variant_product) {
            $unit_price = $product->stocks[0]->price; 
        }
...
if (!function_exists('home_base_price')) {
    function home_base_price($product, $formatted = true)
    {
        ...
        // $price += $tax; //commented by DeveloperAK
...
if (!function_exists('home_discounted_base_price')) {
    function home_discounted_base_price($product, $formatted = true)
    {
        ...
        // $price += $tax; // comment by developerAK
...
if (!function_exists('get_single_product')) {
    ...
    {
        $product_query = Product::query()->with('thumbnail', 'reviews', 'brand', 'stocks', 'user', 'user.shop');
