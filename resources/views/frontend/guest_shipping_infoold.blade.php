...
<script>
document.getElementById('india_pc').addEventListener('input', function() {
    let pincode = this.value;

    if (pincode.length === 6) {
        // Make API call to fetch city and state details
        fetch(`https://api.postalpincode.in/pincode/${pincode}`)
        .then(response => response.json())
        .then(data => {
            if (data[0].Status === "Success") {
                let postOffice = data[0].PostOffice[0];
                document.getElementById('indian_city').value = postOffice.District;
                document.getElementById('indian_state').value = postOffice.State;
            } else {
                alert('Invalid Pincode');
                document.getElementById('indian_city').value = '';
                document.getElementById('indian_state').value = '';
            }
        })
        .catch(error => {
